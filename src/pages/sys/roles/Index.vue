      ...
      <div class="row background-div">
        ...
        <div class="col-xs-6 col-md-4 justify-end">
          <div class="row q-pa-sm q-gutter-sm justify-end">
            <q-btn color="blue-6" icon="add" label="Nuevo" style="box-shadow: 5px 5px 5px #8b8989 !important;" @click="$router.push('/roles/new')" />
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    toogleStatus (row) {
      this.$q.dialog({
        title: 'Confirmación',
        message: row.status === 'ACTIVO' ? '¿Desea dar de baja este Rol?' : '¿Desea reactivar este Rol?',
        persistent: true,
        ok: {
          label: 'Aceptar',
          color: 'positive'
        ...
        api.post(`/roles/toogle-status/${row.id}`).then(() => {
          this.fetchFromServer()
        })
      })
    },
    async qTableRequest (props) {
      ...
      api.get('/roles', params).then(({ data }) => {
        this.$q.loading.hide()
        this.data = data.data
